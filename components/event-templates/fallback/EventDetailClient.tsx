...
import {
  ...
  Tag, XCircle, CheckCircle, Lock,
  Package, CheckCircle2, AlarmClock,
...
// ST42-L01: EventDetailProps and LinkedCampaignSection used to be DECLARED here, in a
// Next.js route file, which forced every template in the component layer to import from
// `@/app/...`. Both now live in the component layer; this file is a consumer like any
// other. The shapes and the rendered markup are unchanged. (The lucide / wizard-type
// imports trimmed above became orphaned by that move — removing them is import-only.)
...
// LinkedCampaignSection → imported from shared/donation/LinkedCampaignSection (ST42-L01)
...
export function EventDetailClient(props: EventDetailProps) {
  ...
  return (
    // ST42-T02: chrome comes from the ONE canonical shell. The 'legacy' variant renders
    // the exact wrapper this component used to inline (bg-background div + navbar, no
    // footer), so the DOM is unchanged; it retires together with this component.
    <EventPageShell variant="legacy" title={title}>
          ...
          <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_420px] lg:gap-8 lg:items-start">
            ...
            <div>
              ...
              <div className="divide-y divide-border/40 bg-background">
                ...
                {linkedCampaign && (
                  <LinkedCampaignSection campaign={linkedCampaign} eventSlug={slug} layout="inline" />
                )}
                ...
                {/* RD-ST15.0: venueType/online deliberately NOT passed — the venue block above already
            ...
            {/* RD-RESPONSIVE-05 — sticky sits on the GRID ITEM, not on a wrapper inside it.
                The parent grid sets `lg:items-start`, which sizes this item to its content;
                a sticky element nested inside it therefore had a containing block exactly its
                own height and zero distance to travel, so it never stuck. As a grid item,
                sticky resolves against the grid CONTAINER instead, giving it the full height
                of the left column to travel while `items-start` keeps the item content-sized.
                `lg:` because the column is `hidden` below that breakpoint anyway. */}
            <div className="hidden lg:block lg:sticky lg:top-24">
              {/* pt-8 aligns card top with left column content. */}
              <div className="pt-8">
