...
/** Draw QR code as a matrix of filled squares — no canvas required. */
function drawQr(
...
export async function GET(
  ...
  // ── Authorization ─────────────────────────────────────────────────────────
  const ticketSecretConfigured = !!process.env.TICKET_SECRET

  // Path 1: HMAC-signed token in query string
  ...
  if (tokenParam) {
    ...
    if (bearerToken) {
      try {
        ...
        // Allow registration owner (if uid stored) or event organizer
        authed = (reg.uid === uid) || (reg.organizerUid === uid)
      } catch {
        // Invalid token — fall through to the TICKET_SECRET check
      }
    }

    if (!authed) {
      if (ticketSecretConfigured) {
        // Strict mode: TICKET_SECRET is set but no valid token or auth was provided
        return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
      }
      // TICKET_SECRET not configured — degrade to legacy UUID-as-token behaviour
      // so existing email links keep working.  Configure TICKET_SECRET in
      // production to enable strict mode.
      console.warn(
        '[tickets/pdf] TICKET_SECRET is not configured. ' +
        'PDF is accessible to anyone with the registrationId. ' +
        'Set TICKET_SECRET to enable signed-link authorization.',
      )
    }
  }
